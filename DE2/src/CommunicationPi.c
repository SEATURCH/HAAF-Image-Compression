...
void writeToHaaf(unsigned char* received, int image_size, unsigned char* filename)
{
	FILE *fp;
	fp = fopen(filename, "w+");
	fwrite(received, 1, image_size, fp);
...
void open_picture(unsigned char *filename)//int main(int argc, char **argv)
{
	FILE *fp;
  ...
  unsigned char sendBuffer[image_size];
	unsigned char* receivedCursor = receivedBuffer;

	//FILE *fp;
	fp = fopen(FILE_YUV, "rb");
	...
	int buf_size = *((int *)receivedCursor);//0x00000000 & receivedBuffer[1] << 2& receivedBuffer[0]
	receivedCursor += sizeof(int);
	if(DEBUG) printf("buf_size  = %i", buf_size);
	writeToHaaf(receivedCursor, buf_size, filename);
